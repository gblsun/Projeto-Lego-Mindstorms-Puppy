...
class Puppy:
    ...
    def eat(self):
        """Comendo: abaixa a cabeça, mastiga e levanta a cabeça de novo."""
    ...
    def go_to_sleep(self):
        ...
        # Acorda sempre que receber carinho ou ver qualquer cor (menos preto,
        # que é o que o sensor costuma ler quando não há nada na frente).
        color = self.color_sensor.color()
        if self.touch_sensor.pressed() or (color is not None and color != Color.BLACK):
            self.count_changed_timer.reset()
    ...
    def go_to_bathroom(self):
        ...
        self.stand_up()
        wait(100)
        # Levanta a perna esquerda e balança 3 vezes.
    ...
    def update_feed_count(self):
        ...
        if color is not None and color != Color.BLACK and color != self.prev_color:
            ...
            if color == Color.RED:
                ...
                # Verde: pedindo comida — senta e chora.
                self.eyes = self.HURT_EYES
                self.sit_down()
                self.ev3.speaker.play_file(SoundFile.DOG_WHINE)
            elif color == Color.BLUE:
                # Azul: língua pra fora e late.
                self.eyes = self.NEUTRAL_EYES
                self.ev3.speaker.play_file(SoundFile.DOG_BARK_2)
            elif color == Color.YELLOW:
                # Amarelo: faz xixi — fica de pé e balança a perna esquerda.
                self.go_to_bathroom()
            else:
                self.eyes = self.SQUINTY_EYES
                self.ev3.speaker.play_file(SoundFile.CRUNCHING)
